...
def _reverse_result(result: LockedGroupResult) -> LockedGroupResult:
...
def load_official_group_results(
    ...
    for index, row in enumerate(raw_results, start=1):
        ...
        team_a_raw = _first_value(row, ["Seleção A", "SeleÃ§Ã£o A"])
        team_b_raw = _first_value(row, ["Seleção B", "SeleÃ§Ã£o B"])
...
def render_official_results_sidebar(
    ...
    can_use = official_results.locked_match_count > 0
    with st.sidebar:
